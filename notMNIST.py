...
def run_bnn_cnn(output):
    ...
    # (3.1, u'1.0.1', '0.2.dev1')
    print("pymc3: {}, theano: {}, lasagne: {}".format(pm.__version__, theano.__version__, lasagne.__version__))
    ...
    def load_dataset():
        ...
        def generate_dataset(folder):
            ...
            np.random.seed(NUM)
            for i in range(NUM):
                fig.add_subplot(COLS, np.ceil(NUM / float(COLS)), i + 1)
    ...
    plt.savefig(os.path.join(data_dir, 'confusion_matrix.png'), bbox_inches='tight')
    plt.close(fig)

    fig = plt.figure()
    pm.traceplot(trace)
    plt.savefig(os.path.join(data_dir, 'trace_plot.png'), bbox_inches='tight')
    plt.close(fig)
